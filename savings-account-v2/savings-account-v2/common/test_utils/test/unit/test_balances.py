...
# common
import common.test_utils.endtoend.balances as balances
from common.test_utils.common.balance_helpers import Balance, BalanceDimensions
...
class BalancesTest(TestCase):
    ...
    def test_standardise_balance_value_time_format(self):
        ...
        for test_case in test_cases:
            result = balances.standardise_balance_value_time_format(
                test_case["timestamp"]
            )

            self.assertEqual(
                result, test_case["expected_result"], test_case["description"]
            )
    ...
    def test_create_balance_dict(self):
        ...
        for dimension, balance_object in expected_balance_dict.items():
            self.assertEqual(
                balance_object.value_timestamp, result[dimension].value_timestamp
            )
