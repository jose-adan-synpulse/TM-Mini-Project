...
address_details = [
    ...
    AddressDetails(
        account_address="Food", description="An address for food purchases.", tags=[]
    ),
    AddressDetails(
...
def execution_schedules():
    return [
        ("CHECK_MAINTENANCE_FEE", {"day": "28", "hour": "23", "minute": "59"}),
        (
            "CLOSE_ORPHANED_STATEMENT_CYCLE",
            {"year": "1971", "start_date": "1970-01-01", "end_date": "1970-01-01"},
        ),
        (
            "CLOSE_ORPHANED_ZERO_BALANCE",
            {"year": "1971", "start_date": "1970-01-01", "end_date": "1970-01-01"},
        ),
        (
            "PUBLISH_EXTRACT",
...
def post_parameter_change_code(old_parameters, new_parameters, effective_date):
    if _has_parameter_value_changed("orphaned", old_parameters, new_parameters):
        ...
        if orphaned == "yes":
            ...
            statement_day = int(
                vault.get_parameter_timeseries(name="statement_day").latest()
            )
            ...
            if _is_zero_balance(balances, denomination):
                # start schedule
                zero_bal_timeout = vault.get_parameter_timeseries(
                    name="zero_bal_timeout"
                ).latest()
...
def pre_posting_code(postings, effective_date):
    ...
    if (
        ...
        + posting_committed_amount
        + posting_pending_outbound_amount
        < 0
    ):
        raise Rejected(
...
def _get_available_balance(
    ...
    for phase in phases_to_include:
        available_balance += balances[
            balance_address, DEFAULT_ASSET, denomination, phase
        ].net
...
def _total_ringfenced_for_category(
    balances, category, phase, denomination, client_transactions
):
    """
