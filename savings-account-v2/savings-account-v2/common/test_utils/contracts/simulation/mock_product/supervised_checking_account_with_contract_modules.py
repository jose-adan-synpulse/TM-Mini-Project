...
def post_posting_code(postings, effective_date):
    denomination = vault.modules["interest"].get_parameter(vault, "denomination")
    overdraft_buffer = vault.modules["interest"].get_parameter(
        vault, "overdraft_buffer"
    )
    overdraft_fee = Decimal(
        vault.modules["interest"].get_parameter(vault, "overdraft_fee")
    )
    overdraft_fee_accrual = vault.modules["interest"].get_parameter(
        vault, "overdraft_fee_accrual"
    )
...
def _check_maintenance_fee(vault, effective_date):
    ...
    maintenance_fee = Decimal(
        vault.modules["interest"].get_parameter(
            vault, "maintenance_fee", effective_date
        )
    )
