...
def execution_schedules():
    ...
    return [
        ("APPLY_ACCRUED_OVERDRAFT_FEES", {"hour": "23", "minute": "50"}),
        (
            "CHECK_MAINTENANCE_FEE",
            {"day": str(maintenance_fee_check_day), "hour": "23", "minute": "55"},
        ),
        (
            "PUBLISH_EXTRACT",
            {
...
@requires(
    event_type="APPLY_ACCRUED_OVERDRAFT_FEES", parameters=True, balances="1 day live"
)
@requires(
...
def post_parameter_change_code(old_parameters, new_parameters, effective_date):
    if _has_parameter_value_changed(
        "maintenance_fee_check_day", old_parameters, new_parameters
    ):
        ...
        vault.amend_schedule(
            event_type="CHECK_MAINTENANCE_FEE", new_schedule=new_schedule
        )
...
def post_posting_code(postings, effective_date):
    ...
    overdraft_fee = Decimal(
        vault.get_parameter_timeseries(name="overdraft_fee").latest()
    )
    overdraft_fee_accrual = vault.get_parameter_timeseries(
        name="overdraft_fee_accrual"
    ).latest()
    ...
    balances = vault.get_balance_timeseries().at(
        timestamp=effective_date + timedelta(seconds=-1)
    )
...
def _apply_accrued_overdraft_fees(vault, effective_date):
    ...
    if overdraft_fee_balance < 0:
        overdraft_buffer = vault.get_parameter_timeseries(
            name="overdraft_buffer"
        ).latest()
        internal_overdraft_fee_account = vault.get_parameter_timeseries(
            name="internal_overdraft_fee_account"
...
def _check_maintenance_fee(vault, effective_date):
    ...
    maintenance_fee = Decimal(
        vault.get_parameter_timeseries(name="maintenance_fee").at(
    ...
    if maintenance_fee > 0:
        ...
        minimum_monthly_deposit = vault.get_parameter_timeseries(
            name="minimum_monthly_deposit"
        ).at(timestamp=effective_date)
