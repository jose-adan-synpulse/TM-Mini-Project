...
_SUPERVISEE_CHECKING_VERSION_ID = "1"
_SUPERVISEE_SAVINGS_VERSION_ID = "2"
_SUPERVISEE_YOUTH_VERSION_ID = "3"

supervised_smart_contracts = [
    SmartContractDescriptor(
        alias="checking", smart_contract_version_id=_SUPERVISEE_CHECKING_VERSION_ID
    ),
    SmartContractDescriptor(
        alias="savings", smart_contract_version_id=_SUPERVISEE_SAVINGS_VERSION_ID
    ),
    SmartContractDescriptor(
        alias="youth", smart_contract_version_id=_SUPERVISEE_YOUTH_VERSION_ID
    ),
]
...
def _apply_maintenance_fee(vault, effective_date):
    ...
    other_accounts = [
        v for (k, v) in vault.supervisees.items() if k != checking.account_id
    ]
    ...
    maintenance_fee = Decimal(
        checking.get_parameter_timeseries(name="maintenance_fee").at(
            timestamp=effective_date
        )
    )
    ...
    checking_pib_directives = (
        checking.get_hook_directives().posting_instruction_batch_directives
    )
    ...
    if not other_accounts:
        ...
        checking_available_balance = _get_available_balance(
            checking, effective_date, denomination
        )
        other_available_balance = 0
        for account in other_accounts:
            other_available_balance += _get_available_balance(
                account, effective_date, denomination
            )
...
def _publish_combined_extract(vault, effective_date):
    ...
    for account in all_accounts:
        ...
        extract_period = timedelta(
            days=1
        )  # Don't currently have access to supervisor schedules
        ...
        pib_data = {
            str(pib.value_timestamp): [
                ...
                "%0.2f"
                % _get_phase_balance(account, pib.value_timestamp, denomination),
            ]
            for pib in batches
        ...
        opening_committed_bal = _get_phase_balance(
            account, opening_timestamp, denomination
        )
        closing_timestamp = effective_date
        closing_available_bal = _get_available_balance(
            account, closing_timestamp, denomination
        )
        closing_committed_bal = _get_phase_balance(
            account, closing_timestamp, denomination
        )
...
def _check_maintenance_fee_in_batch(pib, fee_amount, from_account_id):
    ...
    for post in pib:
        if (
            post.account_id == from_account_id
            and post.account_address == DEFAULT_ADDRESS
        ):
...
def _get_phase_balance(vault, effective_date, denomination, phase=Phase.COMMITTED):
    ...
    if vault.get_alias() == "youth":
        ...
        phase_balance_net = balances[
            (DEFAULT_ADDRESS, DEFAULT_ASSET, denomination, phase)
        ].net
