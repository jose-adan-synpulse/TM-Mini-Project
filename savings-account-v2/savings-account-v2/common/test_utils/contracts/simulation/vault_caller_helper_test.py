...
from common.test_utils.contracts.unit.types_extension import NumberShape
...
contract_modules_basepath = "common/test_utils/common/contract_modules_examples"
CONFIG_FILE = basepath + "/test_config.json"
CONTRACT_EMPTY_FILE = basepath + "/mock_product/empty_contract.py"
CONTRACT_FULL_FILE = basepath + "/mock_product/full_contract.py"
CONTRACT_FULL_UPDATED_VERSION_FILE = (
    basepath + "/mock_product/full_contract_updated_version.py"
)
...
SAVINGS_CONTRACT_WITH_MODULES_FILE = (
    basepath
    + "/mock_product/supervised_savings_deposit_account_with_contract_modules.py"
)
...
class ClientTest(SimulationTestCase):
    @classmethod
    def setUpClass(cls):
        cls.contract_empty = CONTRACT_EMPTY_FILE
        ...
        with open(config, encoding="utf-8") as test_config:
    ...
    def test_supervisor_contract(self):
        ...
        events.append(
            create_account_plan_assoc_instruction(
                timestamp=start + timedelta(days=11),
                id_for_account="Supervised checking account",
                account_id="Checking Account",
                plan_id="1",
            )
        )
        ...
        events.append(
            create_account_plan_assoc_instruction(
                timestamp=start + timedelta(days=11),
                id_for_account="Supervised savings account",
                account_id="Savings Account",
                plan_id="1",
            )
        )
    ...
    def test_set_flag(self):
        ...
        self.assertIn(
            'created flag with definition "1" for account "Main account"', account_logs
        )
    ...
    def test_workflow_instantiation(self):
        ...
        all_workflows = get_instantiated_workflows(res=res)

        self.assertEquals(
            "MOCK_INTEREST_ACCRUAL_WORKFLOW",
            all_workflows.latest()[0]["workflow_definition_id"],
        )

        self.assertEqual(62, len(all_workflows.all()))

        workflows_fee = get_workflows_by_id(
            res=res,
            workflow_definition_id="MOCK_FULL_CONTRACT_OVERDRAFT_FEE_NOTIFICATION",
        )
        self.assertEquals(
            "MOCK_FULL_CONTRACT_OVERDRAFT_FEE_NOTIFICATION",
            workflows_fee.latest()["workflow_definition_id"],
        )
        self.assertEqual(1, len(workflows_fee.all()))
    ...
    def test_manual_event(self):
        ...
        self.assertIn(
            'created account "Manual account"', get_account_logs(res, "Manual account")
        )
        self.assertEqual(get_num_postings(res), 1)
    ...
    def test_posting_instructions(self):
        ...
        self.assertEqual(
            main_balances.at(start + timedelta(minutes=1))[BalanceDimensions()].net, 1
        )
        self.assertEqual(
            main_balances.at(start + timedelta(minutes=2))[BalanceDimensions()].net, 0
        )
        self.assertEqual(
            main_balances.at(start + timedelta(minutes=3))[BalanceDimensions()].net, 2
        )
        self.assertEqual(
            main_balances.at(start + timedelta(minutes=4))[BalanceDimensions()].net, 5
        )
        self.assertEqual(
    ...
    def test_query_internal_account_balances(self):
        ...
        balances = get_balances(res)
        self.assertEqual(
            balances["internal_asset_account_1"].latest()[BalanceDimensions()].net, 1
        )
        self.assertEqual(
            balances["internal_asset_account_2"].latest()[BalanceDimensions()].net, -2
        )
        self.assertEqual(
            balances["internal_asset_account_3"].latest()[BalanceDimensions()].net, 3
        )
        self.assertEqual(
    ...
    def test_create_calendar_and_events(self):
        ...
        self.assertIn(
            'created calendar event for calendar "TEST_CALENDAR"', result_logs
        )
    ...
    def test_check_posting_rejections(self):
        ...
        rejections_with_wrong_time = [
            ExpectedRejection(
                timestamp=datetime(
                    year=2021, month=1, day=1, hour=3, tzinfo=timezone.utc
                ),
                account_id=account_id,
                rejection_type=rejection_type,
                rejection_reason=rejection_reason,
        ...
        with self.assertRaises(AssertionError) as ex:
            self.check_posting_rejections(
                rejections_with_wrong_time, logs_with_timestamp, msg
            )
    ...
    def test_check_schedule_processed(self):
        ...
        schedule_runs_with_wrong_time = [
            ExpectedSchedule(
                run_times=[
                    datetime(year=2021, month=1, day=1, hour=3, tzinfo=timezone.utc)
                ],
                event_id=event_id,
                account_id=account_id,
            )
    ...
    def test_account_update_close_triggers_close_code_0_balance(self):
        start = datetime(year=2019, month=1, day=1, tzinfo=timezone.utc)
        end = datetime(year=2019, month=1, day=2, tzinfo=timezone.utc)
        template_params = {
            "denomination": "GBP",
        ...
        notes = get_account_notes(res, "Main account")
        for note in notes:
            if note["effective_timestamp"] == "2019-01-02T00:10:00Z":
                self.assertIn("Account note close_code", note["body"])
    ...
    def test_account_update_close_triggers_close_code_outstanding_balance(self):
        ...
        self.assertIn(
            "Cannot close account until account balance nets to 0", str(ex.exception)
        )
    ...
    def test_global_parameter(self):
        ...
        events = [
            create_global_parameter_instruction(
                ...
                number=NumberShape(kind="NUMBER_FIELD_DISPLAY_STYLE_PLAIN"),
    ...
    def test_account_product_version_update_triggers_upgrade_code(self):
        ...
        with open(
            self.contract_full_updated_version, encoding="utf-8"
        ) as smart_contract_file:
        ...
        notes = get_account_notes(res, "Main account")
        upgrade_code_account_notes = [
            note for note in notes if "Account note upgrade_code" in note["body"]
        ]
        self.assertEqual(len(upgrade_code_account_notes), 1)
    ...
    def test_contract_module_gets_executed(self):
        ...
        self.assertEqual(
            balances[internal_account_id].latest()[BalanceDimensions()].net, -997
        )
    ...
    def test_supervisor_contract_one_supervisee_with_contract_modules(self):
        ...
        self.assertTrue(
            get_plan_assoc_created(res, plan_id="1", account_id="savings 0")
        )
        self.assertTrue(
            get_plan_assoc_created(res, plan_id="1", account_id="checking 0")
        )
        self.assertTrue(get_plan_assoc_created(res, plan_id="1", account_id="youth 0"))
        self.assertTrue(
            get_module_link_created(res, ["interest"], smart_contract_version_id="1")
        )
    ...
    def test_multiple_supervisees_on_supervisor_plan_with_contract_modules(self):
        ...
        self.assertTrue(
            get_plan_assoc_created(res, plan_id="1", account_id="checking 1")
        )
        self.assertTrue(get_plan_assoc_created(res, plan_id="1", account_id="youth 0"))
        ...
        self.assertTrue(
            get_module_link_created(
                res, ["interest", "module_2"], smart_contract_version_id="2"
            )
        )
